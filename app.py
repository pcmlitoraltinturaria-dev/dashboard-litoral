...
# Refresh Automático de 30 segundos
...
# 2. CSS Profissional (Ajuste de Letras, Espaçamento Superior e Cores)
st.markdown(f"""
    <style>
    /* Baixa o conteúdo do topo da página */
    .block-container {{ 
    ...
    .card {{
        ...
        border-top: 7px solid; /* Espessura idêntica para todas */
    }}

    /* Tamanho das Letras - Ajustado para não encavalar */
    .nome-topo {{ 
        color: #a0aec0; 
        font-size: 0.65rem; 
        font-weight: 700; 
    ...
    .id-numeros {{ 
        font-size: 2.3rem; /* Tamanho ideal para 4 dígitos */
        font-weight: 900;
        color: #ffffff;
        line-height: 1;
        letter-spacing: -1px;
...
cols = st.columns(8) # Grid de 8 colunas para melhor visibilidade

# 4. Processamento e Exibição
for idx, at in enumerate(ativos):
    # Procura o ID da máquina no texto bruto do Firebase
    pos = string_bruta.rfind(at['id'])
    
    # REGRA: Se não houver informação de parada no texto, o padrão é sempre VERDE (NORMAL)
    status, cor, classe, icon, servico = "NORMAL", "#2ecc71", "border-normal", "✅", "EM OPERAÇÃO"
    
    if pos != -1:
        # Pega o contexto próximo ao nome da máquina
        ctx = string_bruta[pos : pos + 400]
        
        # Filtro Rigoroso: Só muda se encontrar o texto exato
        ...
        # O Químico 1602, sendo "Normal" no texto, cairá automaticamente no padrão VERDE (NORMAL)

    # Limpeza do ID para exibição (remove caracteres internos)
